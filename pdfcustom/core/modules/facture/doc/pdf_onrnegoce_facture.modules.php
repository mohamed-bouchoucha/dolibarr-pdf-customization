...
class pdf_onrnegoce_facture extends ModelePDFFactures
{
    public $db;
    public $name = "onrnegoce_facture";
    public $description;
    public $page_largeur;
    public $page_hauteur;
    public $format;
    ...
    public function __construct($db)
    {
        global $langs;
        $this->db = $db;
        $this->description = "Modèle Premium ONR Négoce pour Factures";
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs) 
    {
        ...
        $pdf->MultiCell(70, 8, "FACTURE ".$object->ref, 0, 'C', 1);
    ...
    protected function _extrafieldsBlock(&$pdf, $object, $outputlangs)
    {
        ...
        if (!empty($val_ref_cmd) || !empty($val_mode_liv)) {
            ...
            $pdf->SetFont('', '', 9);
            if($val_ref_cmd) { $pdf->SetX(10); $pdf->SetFont('', 'B'); $pdf->Cell(40, 5, "Réf Commande :"); $pdf->SetFont('', ''); $pdf->Cell(50, 5, $val_ref_cmd, 0, 1); }
            if($val_mode_liv) { $pdf->SetX(10); $pdf->SetFont('', 'B'); $pdf->Cell(40, 5, "Mode Livraison :"); $pdf->SetFont('', ''); $pdf->Cell(50, 5, $extrafields->showOutputField('mode_livraison', $val_mode_liv), 0, 1); }
        }
    }
    ...
    protected function _pagefoot(&$pdf, $object, $outputlangs)
    {
        ...
        $pdf->MultiCell(0, 4, $mysoc->name." - CP : ".$mysoc->zip." ".$mysoc->town." - TVA : ".$mysoc->tva_intra, 0, 'C');
